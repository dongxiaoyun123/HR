<template>
  <div style="margin:8px">
            ...
            <el-col :span="6">
              <el-form-item style="margin-bottom: 0;" label="付款方">
                <el-select v-model="WhereParameter.EnterPriseCode" class="whereClass"
                           filterable placeholder="付款方" @change="GetAdmin_PermissionSearch"
                >
            ...
            <el-col :span="12">
              <el-form-item style="margin-bottom: 0;" label="使用状态">
                <el-select v-model="WhereParameter.States" filterable placeholder="使用状态" clearable>
                  ...
                  >
                    <template>
                      <el-tag v-if="item.value == 0" effect="plain" type="success">在用方案</el-tag>
                      <el-tag v-if="item.value == 1" effect="plain" type="info">隐藏方案</el-tag>
                    </template>
                  </el-option>
                </el-select>
    ...
    <el-card class="CardTableClass">
      <el-table ref="multipleTable" v-loading="loading" :data="ProgramInfoList" fit highlight-current-row>
        <el-table-column prop="ConProgramID" label="方案编号" min-width="100" show-overflow-tooltip />
        <el-table-column prop="ProgramNameOld" label="方案名称" min-width="150" show-overflow-tooltip />
        ...
        <el-table-column prop="IsDel" label="使用状态" min-width="100">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.IsDel == 0" effect="plain" type="success">在用方案</el-tag>
            <el-tag v-if="scope.row.IsDel == 1" effect="plain" type="info">隐藏方案</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="OrderBy" label="排序" min-width="80" />
        <el-table-column prop="PlanType" label="方案类型" :formatter="PlanTypeFormat" min-width="100" />
