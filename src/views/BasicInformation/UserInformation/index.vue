    ...
    <div style="margin:8px">
        ...
        <el-card class="CardTableClass">
            <el-table v-loading="loading" highlight-current-row :data="UserList" fit>
                <el-table-column prop="User_Account" label="登陆用户名" min-width="100" show-overflow-tooltip></el-table-column>
                <el-table-column prop="User_RealName" label="真实姓名" min-width="150" show-overflow-tooltip></el-table-column>
        ...
        <!-- 添加用户 -->
        <el-dialog :visible.sync="addUserVisible" top="5vh" width="45%" @close="detailUserVisibleClosed"
            ...
            <el-form :model="AddUserForm" ref="addStaffRef" :rules="addContractorRules" label-width="120px">
                <el-alert style="margin-bottom:20px ;" type="success" show-icon v-if="!IfUpdate"
                    title="此登陆密码默认为登录名加“YUANFU001”，例如：用户名为dongxiaoyun则密码为dongxiaoyunYUANFU001" :closable="false">
                </el-alert>
                <el-form-item label="用户类型" prop="UserType">
                    <el-radio :disabled="IfUpdate" @input="UserTypeChange" v-model="AddUserForm.UserType" label="1"
                        border>合同方用户</el-radio>
                    <el-radio :disabled="IfUpdate" @input="UserTypeChange" v-model="AddUserForm.UserType" label="2"
                        border>付款方用户</el-radio>
                </el-form-item>
                ...
                <el-form-item label="登陆用户名" prop="User_Account">
                    <el-input v-model="AddUserForm.User_Account" placeholder="登陆用户名"></el-input>
                </el-form-item>
                <el-form-item label="真实姓名" prop="User_RealName">
                    <el-input v-model="AddUserForm.User_RealName" placeholder="真实姓名"></el-input>
                </el-form-item>
                ...
                <el-form-item label="演示用户">
                    <el-switch :disabled="IfUpdate" v-model="AddUserForm.DemoUser" active-text="是" inactive-text="否">
                    </el-switch>
                </el-form-item>
...
export default {
    ...
    data() {
        ...
        var checkEnterPriseCode = (rule, value, callback) => {
            if (!this.ShowEntFlag) {
                if (!this.AddUserForm.EnterPriseCode)
                    callback("请选择付款方");
                else
                    callback();
            }
        ...
        GetChildEnterprise(flag) {
            GetChildEnterprise(this.AddUserForm.ParentCode).then((res) => {
                if (res.success) {
                    ...
                    if (!flag)
                        this.AddUserForm.EnterPriseCode = res.result[0].EnterPriseCode;
                } else {
                    this.ChildEnterpriseList = [];
                }
            });
